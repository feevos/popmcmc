...
#ifndef _DES_PCX_stepper_
#define _DES_PCX_stepper_
...
class DES_PCX_stepper{
	...
	public: 
		...
		DES_PCX_stepper(vector<double> &_weights, std::vector<std::vector<double> > &_PriorRange, const int &_Nvars, const int &_Npop, int _mu=5);
		~DES_PCX_stepper(); 
...
template <class T>
DES_PCX_stepper<T>::~DES_PCX_stepper(){
	delete stepper1;
	delete stepper3;
}
...
template <class T>
T DES_PCX_stepper<T>::propose( std::vector<T> &X, int &j) {
