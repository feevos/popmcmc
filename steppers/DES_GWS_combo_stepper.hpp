...
// TODO: GW stepper has different MH ratio from DES, so a proposal needs to be combatible with specific mh_ratio 



#ifndef _DES_GWS_stepper_
#define _DES_GWS_stepper_
...
class DES_GWS_stepper{
	...
	public: 
		...
		DES_GWS_stepper(vector<double> &_weights, const int &_Nvars, const int &_Npop, int _mu=5);
		~DES_GWS_stepper(); 
...
template <class T>
DES_GWS_stepper<T>::DES_GWS_stepper(vector<double> &_weights, const int &_Nvars, const int &_Npop, int _mu):weights(_weights){
...
template <class T>
DES_GWS_stepper<T>::~DES_GWS_stepper(){
	delete stepper1;
	delete stepper2;
}
...
template <class T>
T DES_GWS_stepper<T>::propose( std::vector<T> &X, int &j) {
